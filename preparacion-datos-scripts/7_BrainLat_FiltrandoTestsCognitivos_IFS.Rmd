---
title: 'BrainLat: Filtrando Cognition (IFS)'
author: "Yassmina"
date: "2025-05-16"
...
```{r}
# Eliminamos columnas repetidas, casi vacías y los MOCA (pq. vimos que brindaban la misma información que los IFS y tienen más faltantes)
reco <- reco %>% select(-c(EEG_ID, diagnosis, country)) 
cogn <- cogn %>% select(-c(EEG_ID, diagnosis, emotion.recog, country, moca_total, moca_visuospatial, moca_recog, moca_attention, 
...
missing_table <- merged_data %>%
  group_by(diagnosis) %>%
  summarise(porcentaje_faltantes = mean(rowMeans(is.na(across(9:19)))) * 100)
...
```{r}
# Han de tener al menos el 75% de los valores demosgráficos + resultados de los test cognitivos y al menos una MRI
filtered <- merged_data %>%
  filter(rowMeans(!is.na(across(3:19))) >= 0.75 &  
         (T1 == 1 | Rest == 1 | DWI == 1))         

head(filtered)
```
```{r}
# Nos quedan 454 observaciones, veamos cuántas eeg hay y cuántas MRI hay
...
```{r}
write.csv(filtered, "BrainLat_Filtrado2.csv", row.names = FALSE)
```
